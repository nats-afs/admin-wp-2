    ...
    <v-card-text>
      ...
      <v-tabs v-model="active">
                  ...
                  <v-layout row>
                    <v-flex xs6>
                      ...
                      <v-menu ref="menu"
                        ...
                        :nudge-right="40"
                        min-width="290px"
                        :return-value.sync="item.dateInit">
                        <v-text-field slot="activator"
                          label="Selecccionar fecha"
                          v-model="item.dateInit"
                          :error-messages="dateErrors"
                          prepend-icon="event"
                          readonly></v-text-field>
                        <v-date-picker v-model="item.dateInit"
                          :reactive="true"
                          no-title
                          scrollable>
                          <v-spacer></v-spacer>
                          ...
                          <v-btn flat
                            color="primary"
                            @click="$refs.menu.save(item.dateInit)">OK</v-btn>
                        </v-date-picker>
                      </v-menu>
                    </v-flex>
                    <v-flex xs6>
                      <v-text-field name="image"
                        label="Seleccione una imagen"
                        v-model.lazy="item.imageName"
                        prepend-icon="image"
                      ...
                      <input type="file"
                        ...
                        @change="onFilePicked">
                      <v-card-media :src="item.imageValue"
        ...
        <v-tab-item>
          <v-text-field name="image"
            label="Imagenes"
            :value="multipleImagesText"
            prepend-icon="image"
            @click='pickGallery'
            hint="Seleccione multiples imagenes"></v-text-field>
          <input type="file"
            style="display: none"
            ref="gallery"
            multiple
                ...
                <v-card>
                  <v-container fluid
                    ...
                    <v-layout row
                      ...
                      <v-flex xs12
                        sm3
                        v-for="(picture,index) in item.gallery"
                        :key="picture.id">
                        <v-card flat
                          tile>
                          <v-card-media :src="picture.src"
                            height="150px">
                          </v-card-media>
                          <v-text-field name="caption"
                            labell="Caption"
                            v-model.lazy="item.gallery[index].caption"></v-text-field>
                          <v-btn color="blue"
                            dark
                            small
                            absolute
