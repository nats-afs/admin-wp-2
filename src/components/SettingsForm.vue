                ...
                <v-form>
                  <v-layout row>
                    <v-flex xs6>
                      ...
                      <v-text-field name="alcalde" label="Alcalde" v-model="item.alcalde" :error-messages="alcaldeErrors" @input="$v.item.alcalde.$touch()" @blur="$v.item.alcalde.$touch()"></v-text-field>
...
export default {
  ...
  validations: {
    item: {
      ...
      alcalde: { required, minLength: minLength(5) },
  ...
  methods: {
    fetchOrg(data) {
      ...
      this.item.alcalde = data.alcalde;
  ...
  computed: {
    ...
    alcaldeErrors() {
      const errors = [];
      if (!this.$v.$dirty) return errors;
      !this.$v.item.alcalde.minLength &&
        errors.push("Alcalde debe tener un minimo 5 caracteres");
      !this.$v.item.alcalde.required && errors.push("Alcalde es requerido.");
      return errors;
    },
    sloganErrors() {
      const errors = [];
