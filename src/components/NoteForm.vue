          ...
          <v-flex xs12>
            <v-form>
              <v-text-field label="Nombre"
                ...
                @blur="$v.item.name.$touch()"></v-text-field>
              <v-text-field label="Descripcion"
                v-model="item.description"
                multi-line
                :error-messages="descriptionErrors"
                @input="$v.item.description.$touch()"
                @blur="$v.item.description.$touch()"></v-text-field>
              <v-text-field label="Contenido"
                v-model="child"
                @keyup.enter="addChild"
                :error-messages="childErrors"
                @input="$v.child.$touch()"
                @blur="$v.child.$touch()"></v-text-field>
              <v-list v-model="item.children">
                <v-list-tile v-for="(note , index ) in item.children"
                  :key="note.id">
                  <v-list-tile-content>
                    <v-text-field solo
                      v-model="item.children[index].name"></v-text-field>
                  </v-list-tile-content>
                  <v-list-tile-action>
                    <v-btn icon
                      @click="remove(index)">
...
import { fb } from "../config/facebookConfig";
export default {
  props: {
    edit: {
      type: Boolean,
  ...
  validations: {
    ...
    child: { minLength: minLength(5), maxLength: maxLength(30) }
  },
  data: () => ({
    child: null,
    alert: false,
  ...
  computed: {
    ...
    nameErrors() {
      ...
      !this.$v.item.name.maxLength &&
        errors.push("El nombre de nota debe tener maximo 30 caracteres");
    ...
    descriptionErrors() {
      ...
      !this.$v.item.description.maxLength &&
        errors.push("Descripcion debe tener maximo de 50 caracteres");
    ...
    childErrors() {
      ...
      !this.$v.child.maxLength && errors.push("Maximo 30 caracteres");
      return errors;
    }
  }
};
